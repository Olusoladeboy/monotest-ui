...
import dash from "../../assets/images/mono-dash.svg";
...
import link_account from "../../lib/apiHandlers/link_account";
...
export function LinkAccount() {
  ...
  const linkaccount = async (code) => {
    try {
      await link_account({ code }).then(async (res) => {
        console.log(res);
        if (res.success) {
          await getuser()
            .then((res) => {
              console.log(res);
              storage.addItem("@user-user", res);
            })
            .catch((err) => {
              loadError(err);
              console.log(err);
  ...
  const monoConnect = React.useMemo(() => {
    const monoInstance = new MonoConnect({
      ...
      //   link_account(code)
      // }),
      onSuccess: ({ code }) => linkaccount(code),
  ...
  // console.log(monoConnect.onSuccess());
  return (
    <div className="flex flex-no-wrap h-screen">
      {/* Sidebar starts */}
      {/* Remove class [ hidden ] and replace [ sm:flex ] with [ flex ] */}
      <div className="w-64 h-screen absolute sm:relative bg-black shadow md:h-full flex-col justify-between hidden sm:flex">
        <div className="px-8">
      ...
      <div className="container mx-auto py-10 md:w-4/5 w-11/12 px-6 h-full">
        ...
        <div className="flex h-full">
          <div className="link-account-card m-auto">
            <div className="w-full">
              <div className="content-center py-10">
                <img className="mx-auto" src={padlock} alt="" />
                <div>
                  <h2 className="h2">Final Step</h2>
                </div>
                <p className="p mt-4">Link your Bank Account in seconds</p>
                {/* <Link to="/dashboard"> */}
                <button
                  onClick={() => monoConnect.open()}
                  className="mt-10 flex mx-auto items-center px-5 py-2 gap-1"
                >
                  <span>Link now </span>
                  <span>
